...
BEGIN_MESSAGE_MAP(CViewTimeline, CScrollView)
	...
	ON_WM_TIMER()
	ON_COMMAND(ID_PLAY_PLAY32789, &CViewTimeline::OnPlayPlay32789)
	ON_COMMAND(ID_PLAY_PLAYFROMBEGINNING32790, &CViewTimeline::OnPlayPlayfrombeginning32790)
 ...
 */
 void CViewTimeline::OnTimer(UINT_PTR nIDEvent)
 {
	 ...
	 */
	 ...
	 // Add elapsed time to picture animation time 
	 picture->SetAnimationTime(picture->GetAnimationTime() + elapsed);
	 
	 // Stop animation if the duration is less than/ equal to animation time
	 if (duration <= picture->GetAnimationTime())
	 {
		 OnPlayStop32791();

		 // Set animation time to timeline duration
		 picture->SetAnimationTime(duration);
	 }
...
/**
 * Play animation from current pointer position
 */
 void CViewTimeline::OnPlayPlay32789()
 {
	 ...
	 if (mIsPlaying == false)
	 {
		 ...
		 */
		 ...
		 SetTimer(1, timeline->GetDuration(), nullptr);
	 }
 }
 ...
 */
 void CViewTimeline::OnPlayPlayfrombeginning32790()
 {
	 auto picture = GetPicture();
